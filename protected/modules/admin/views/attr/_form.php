<div class="form">
    ...
    <?php
                    echo CHtml::dropDownList('category_id',
                                             $model->isNewRecord ? '':$model->attrGroup->category_id,
                                             Category::TreeArray(false),
                                             array(
                                                  'empty' => 'выберите категорию',
                                                  'encode' => false,
                                                  'onchange' => 'GetCategoryGroups(this.value, \'' . $this->createUrl('group/CategoryGroupsOptions') . '\')',
    ...
    <?php
        echo $form->dropDownList($model, 'attr_group_id',
                                 $model->isNewRecord ?
                                         array() :
                                         CHtml::listData(
                                             AttrGroup::model()->findAll('category_id=' . $model->attrGroup->category_id), 'id', 'name'),
                                 array(
    ...
    <div class="row">
        ...
        <?php echo $form->labelEx($model, 'is_main', array('class' => 'inline-label')); ?>
        <?php echo $form->error($model, 'is_main'); ?>
    </div>

    <div class="row">
        <?php echo $form->checkBox($model, 'filter'); ?>
        <?php echo $form->labelEx($model, 'filter', array('class' => 'inline-label')); ?>
